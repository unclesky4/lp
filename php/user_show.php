<?php header('Content-type:text/html;charset=UTF-8');   //显示用户（管理员）信息
	session_start();
	require "sql.php";
	if(isset($_SESSION['USER'])){
		...
		if(isset($order_column)){
		    ...
		    switch($i){
		        ...
		        case 1: $orderSql = " order by uname ".$order_dir; break;
		...
		$sumSqlWhere = " where id like '%".$search."%' and uname like '%".$search."%' ";
		if(strlen($search)>0){
		...
		if(strlen($search)>0) {
		    ...
		    while ($row = $dataResult->fetch_assoc()) {
		        $obj = array($row['id'], $row['uname'], $row['password']);
		        array_push($data, $obj);
		    }
		}else{
		    //直接查询所有记录
